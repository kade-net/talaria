...
export class PreKeyBundle {
    ...
    static deriveFromPrivateKey(arg: deriveFromPrivateKeyArg) {
        ...
        const preKey = preKeySecret
            ? nacl.box.keyPair.fromSecretKey(preKeySecret)
            : nacl.box.keyPair();

        const signature = nacl.sign(preKey.publicKey, originalKeyPair.secretKey);
    ...
    static deserialize(serializedKeyBundle: Buffer) {
        assert(
            serializedKeyBundle.length === 160,
            new AssertionError("Invalid Key Bundle"),
        );
        const buff = serializedKeyBundle;
        const identityKey = buff.subarray(0, 32);
        assert(
            identityKey.length == 32,
            new AssertionError("Identity Key too short"),
        );
        const signedPreKey = buff.subarray(32, 64);
        assert(
            signedPreKey.length == 32,
            new AssertionError("Signed Pre Key too short"),
        );
        const signedPreKeySignature = buff.subarray(64, 160);
        assert(
            signedPreKeySignature.length == 96,
            new AssertionError("signedPreKeySignature too short"),
        );
